...
const createEvent = async (req, res) => {
  ...
  try {
    ...
    const {
      ...
      artists,
      ticketPrice,
      images, // Array of objects like [{ url: 's3-key-or-url' }, ...]
      videoUrl, // A single string referencing the S3 file
    } = req.body;
    ...
    // Parse the artists field if it’s a JSON string
    let parsedArtists = [];
    ...
    const newEvent = new Event({
      ...
      images, // Already an array of { url: '...' } from the frontend
      videoUrl, // S3 link or key from the frontend
    ...
    // Add this event to the user's `createdEvents`
    const user = await User.findById(userId);
    if (!user) {
      console.error("User not found with id:", userId);
      return res.status(404).json({ error: "User not found" });
    ...
    console.log("User successfully updated with new event:", user);
    res
      .status(201)
      .json({ message: "Event created successfully", event: newEvent });
  } catch (error) {
